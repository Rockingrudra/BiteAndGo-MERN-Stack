...
router.post("/orderData", async (req, res) => {
  try {
    ...
    if (!email || !order_data) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid order data received" });
    }

    // Flatten the structure: ensure only 1-level nested array
    const formattedOrder = order_data.flat(); // removes that extra array layer
    ...
    if (!existingOrder) {
      ...
      console.log("✅ Created new order entry");
      ...
      console.log("🟢 Added new order for existing user");
      return res.json({ success: true });
    }
  } catch (error) {
